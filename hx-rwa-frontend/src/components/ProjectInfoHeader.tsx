...
export function ProjectInfoHeader() {
  return (
    <div className="mb-4 text-left border-l-4 border-yellow-400 pl-3">
...
export function ProjectInfoStats() {
  ...
  const stats = [
    ...
    { label: "Total Holders", value: randomHolders.toString(), sub: "Unique CAB holders" }
  ];

  return (
    <div className="flex flex-col gap-3 flex-1">
      {stats.map((stat, idx) => (
        <div key={idx} className="bg-white p-4 rounded-lg shadow-sm border-2 border-yellow-400">
          <p className="text-xs font-medium text-gray-800 mb-0.5">{stat.label}</p>
          <p className="text-lg font-bold text-black mb-0.5">{stat.value}</p>
